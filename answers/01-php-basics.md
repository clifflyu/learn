...
| PHP 7 / 8 的优化？ | PHP 7：`zval` 24→16 B，Bucket 从分散 `emalloc` 改连续数组，内存约减半；PHP 8.0 packed array 再省 60% |
...
| 为什么 PHP 7 内存比 5 省一半？ | `zval` 从 24 B 降到 16 B；`zend_string` / 数组 Bucket 也从"分散 emalloc + 指针链表"改成连续内存 |
